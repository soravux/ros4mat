...
void logico_start(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    lConnectMessage.compression = (nrhs > 1 && (uint16_t)mxGetScalar(prhs[1]) == 1) ? MSGID_HEADER_ZLIBCOMPRESSION : MSGID_HEADER_NOCOMPRESSION;
    
    #if defined USE_WINSOCK
        if (init_winsock() != 0)
            mexErrMsgTxt("Winsock initialisation error.");
    #endif
    ...
    if (nrhs < 1) {
        ...
        char ip[16] = {0};
        if (mxGetString(prhs[0], ip, sizeof(ip)-1))
            mexErrMsgTxt("IP extraction impossible");
    ...
    if (lHeader.type != MSGID_CONNECT_ACK)
...
void logico_subscribe(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs < 1)
        mexErrMsgTxt("Please specify a valid node.");
    if (nrhs > 1) {
    ...
    if (nrhs > 6)
        mexWarnMsgTxt("Cannot understand request: Too much arguments.");
    if (nrhs > 4)
    ...
    if (nrhs > 2) {
        if (!mxIsChar(prhs[2])) {
            ...
            if (mxGetString(prhs[2], StrBuffer, sizeof(StrBuffer)-1))
				mexErrMsgTxt("Error: Could not interpretate.");
            ...
            if(nrhs > 5)
                lSubscribeMessage.paramSupp2 = (unsigned char)mxGetScalar(prhs[5]);
        }
    }
    ...
    if (!mxIsChar(prhs[0]))
        mexErrMsgTxt("'subscribe' must be followed with a node name.");
...
void logico_unsubscribe(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!mxIsChar(prhs[0]))
        mexErrMsgTxt("'unsubscribe' must be followed with a node name.");
...
msgHeader logico_send_data_request(char inType, char** msg, unsigned int inStructSize) {
    ...
    *msg = (char*)mxCalloc(1, sizeof(msgHeader) + lHeader.uncompressSize);
    ...
    if(lHeader.compressionType == MSGID_HEADER_NOCOMPRESSION){
		...
		i = ezuncompress(*msg+sizeof(msgHeader), &uncompressSize, (unsigned char*)msgCompress, lHeader.compressSize);
        if (i < 0) {
...
void logico_battery(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for (h = 0; h < lHeader.size; h++) {
        ...
        lPercentBattery = (double)((double)lBatteryMessage.cellsVoltage[0] +
					  ...
					  (double)lBatteryMessage.cellsVoltage[5]) * 0.3 - 6.41;
		...
		for (i = 0; i < 6; i++) {
			if (lBatteryMessage.cellsVoltage[i] < 3.0) { lBatteryMessage.state |= 0x100; }
			if (lBatteryMessage.cellsVoltage[i] < 3.6) { lBatteryMessage.state |= 0x200; }
		}
        ...
        for (y=0, b=0; y < lCam.width * lCam.height * 3 - lCam.width * 3; b++, y += lCam.width * 3) {
            for (x=y, a=b; x < y + lCam.width * 3; a += lCam.height) {
                ...
                out_data_l[a + i * sizeImg + lCam.width * lCam.height * 0] = msg[sizeof(msgHeader) /* Skip the packet header */
                                                                                 ...
                                                                                 + x]; /* Take value */
                out_data_r[a + i * sizeImg + lCam.width * lCam.height * 0] = msg[sizeof(msgHeader)
                                                                                 + sizeof(msgCam) * lHeader.size
                                                                                 + sizeImg * (i * 2 + 1)
                                                                                 + x++];
                out_data_l[a + i * sizeImg + lCam.width * lCam.height * 1] = msg[sizeof(msgHeader)
                                                                                 + sizeof(msgCam) * lHeader.size
                                                                                 + sizeImg * (i * 2)
                                                                                 + x];
                out_data_r[a + i * sizeImg + lCam.width * lCam.height * 1] = msg[sizeof(msgHeader)
                                                                                 + sizeof(msgCam) * lHeader.size
                                                                                 + sizeImg * (i * 2 + 1)
                                                                                 + x++];
                out_data_l[a + i * sizeImg + lCam.width * lCam.height * 2] = msg[sizeof(msgHeader)
                                                                                 + sizeof(msgCam) * lHeader.size
                                                                                 + sizeImg * (i * 2)
                                                                                 + x];
                out_data_r[a + i * sizeImg + lCam.width * lCam.height * 2] = msg[sizeof(msgHeader)
                                                                                 + sizeof(msgCam) * lHeader.size
                                                                                 + sizeImg * (i * 2 + 1)
                                                                                 + x++];
            }
...
void logico_kinect(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	for(i=0; i < lHeader.size; i++) {
		/* On passe sur toutes les images */
		memcpy(&lCam,
			   msg + sizeof(msgHeader) + i * sizeof(msgCam),
			   sizeof(msgCam));
		...
		for (y=0, b=0; y < lCam.width * lCam.height * 4 - lCam.width * 4; b++, y += lCam.width * 4) {
			for (x=y, a=b; x < y + lCam.width * 4; a += lCam.height) {
                ...
                out_data[a + i * sizeImg + lCam.width * lCam.height * 3] = *(unsigned short*)&msg[sizeof(msgHeader)
                                                                                                  + sizeof(msgCam) * lHeader.size
                                                                                                  + sizeImg * i
                                                                                                  + x];
                x += 2;
    ...
    for(i=0; i < lHeader.size; i++) {
        ...
        for (y=0, b=0; y < lCam.width * lCam.height - lCam.width; b++, y += lCam.width) {
            for (x=y, a=b; x < y + lCam.width; a += lCam.height) {
                out_data[a + i * sizeImg + 0] = *(unsigned short*)&msg[sizeof(msgHeader)
                                                + sizeof(msgCam) * lHeader.size
                                                + 2 * sizeImg * i
                                                + x++ * 2];
            }
...
void logico_hokuyo(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for(i=0; i < lHeader.size; i++) {
        ...
        memcpy(out_data + i * hokuyo_size[0] * sizeof(float), msg + sizeof(msgHeader) + lHeader.size * sizeof(msgHokuyo) + i*hokuyo_size[0]*sizeof(float), hokuyo_size[0]*sizeof(float));
