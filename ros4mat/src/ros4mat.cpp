...
void dataAdcReceived(const ros4mat::M_ADC::ConstPtr& msg)
{
    ...
    for (int i = 0; i < msg->timestamps.size(); i++) {
        for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
            if((*lClientIt).second.subscribers.count(MSGID_ADC) == 0)
                continue;
            lMsg = new msgAdc;
...
void dataImuReceived(const ros4mat::M_IMU::ConstPtr& msg)
{
    ...
    for (unsigned int i = 0; i < msg->timestamps.size(); i++) {
        for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
            if((*lClientIt).second.subscribers.count(MSGID_IMU) == 0)
                    continue;
...
void dataCamReceived(const sensor_msgs::Image::ConstPtr& msg){ 
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_WEBCAM) == 0)
            continue;
        ...
        if(lMsg->sizeData != lMsg->width*lMsg->height*3) {
            ROS_WARN("Incoherence de taille des buffers de camera (taille prevue : %d, obtenue : %d", lMsg->width*lMsg->height*3, lMsg->sizeData);
        }
        ...
        lMsg = NULL;  
        while ((*lClientIt).second.subscribers[MSGID_WEBCAM].second.size() > (*lClientIt).second.buffersInfo[MSGID_WEBCAM]) {
...
void dataStereoCamReceived(const ros4mat::M_StereoCam::ConstPtr& image){
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_WEBCAM_STEREO) == 0) 
            continue;
        ...
        if((image->image_left).size() != (image->image_right).size()){
            ROS_WARN("Incoherence de taille entre camera gauche et droite (gauche : %d, droite : %d )", (image->image_left).size(), (image->image_right).size());  
        }
        ...
        while ((*lClientIt).second.subscribers[MSGID_WEBCAM_STEREO].second.size() > (*lClientIt).second.buffersInfo[MSGID_WEBCAM_STEREO]) { 
...
void dataGpsReceived(const gps_common::GPSFix::ConstPtr& msg){
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_GPS) == 0)
                        continue;
...
void dataBatteryReceived(const ros4mat::M_Battery::ConstPtr& msg){
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
            if((*lClientIt).second.subscribers.count(MSGID_BATTERY) == 0)
                            continue;
            ...
            for(unsigned int i = 0; i < 6; i++)
                    lMsg->cellsVoltage[i] = msg->vcell[i];
            
            lMsg->state = 0;
            for(unsigned int i = 0; i < 8; i++)
                lMsg->state |= (msg->batteryState[7 - i]) ? 1 << i : 0;
                    
            // Add to the buffer
            (*lClientIt).second.subscribers[MSGID_BATTERY].second.push((void*)lMsg);
            lMsg = NULL;
            // Pruning obsolete data
            while ((*lClientIt).second.subscribers[MSGID_BATTERY].second.size() > (*lClientIt).second.buffersInfo[MSGID_BATTERY]) {
...
void dataComputerReceived(const ros4mat::M_Computer::ConstPtr& msg){
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_COMPUTER) == 0)
                        continue;
        ...
        lMsg->state = 0x0;
        for(unsigned int i = 0; i < msg->cpuLoad.size(); i++)
            lMsg->cpuLoad[i] = msg->cpuLoad[i];
        for(unsigned int i = 0; i < msg->memUsed.size(); i++)
            lMsg->memUsed[i] = msg->memUsed[i];
        for(unsigned int i = 0; i < msg->unixLoad.size(); i++)
            lMsg->unixLoad[i] = msg->unixLoad[i];
        ...
        // Pruning obsolete data
        while ((*lClientIt).second.subscribers[MSGID_COMPUTER].second.size() > (*lClientIt).second.buffersInfo[MSGID_COMPUTER]) {
...
void dataHokuyoReceived(const sensor_msgs::LaserScan::ConstPtr& msg){
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_HOKUYO) == 0)
            continue;
        ...
        lMsg = NULL;
        // Pruning obsolete data
        while ((*lClientIt).second.subscribers[MSGID_HOKUYO].second.size() > (*lClientIt).second.buffersInfo[MSGID_HOKUYO]) {
...
void dataKinectDepthReceived(const sensor_msgs::Image::ConstPtr& msg) {
    ...
    for ( lClientIt = clients.begin() ; lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_KINECT_DEPTH) == 0)
            continue;
        lMsg = new msgCamInternal;
        lMsg->timestamp = msg->header.stamp.toSec();
        lMsg->width = msg->width;
        ...
        if(lMsg->sizeData != lMsg->width * lMsg->height * 1 * 2) {     // * 2 parce que ce sont des uint16
            ROS_WARN("Incoherence de taille des buffers de camera (taille prevue : %d, obtenue : %d", lMsg->width*lMsg->height*1*2, lMsg->sizeData);
        }
        ...
        // Pruning obsolete data
        while ((*lClientIt).second.subscribers[MSGID_KINECT_DEPTH].second.size() > (*lClientIt).second.buffersInfo[MSGID_KINECT_DEPTH]) {
...
void dataKinectRGBReceived(const sensor_msgs::Image::ConstPtr& msg) {
    ...
    for ( lClientIt = clients.begin(); lClientIt != clients.end(); lClientIt++ ) {
        if((*lClientIt).second.subscribers.count(MSGID_KINECT) == 0)
            continue;
        lMsg = new msgCamInternal;
        lMsg->timestamp = msg->header.stamp.toSec();
        lMsg->width = msg->width;
        ...
        if((msg->data).size() != lMsg->width * lMsg->height * 3) {
            ROS_WARN("Incoherence de taille des buffers de camera (taille prevue : %d, obtenue : %d", lMsg->width * lMsg->height * 3, (msg->data).size());
        }
        ...
        // Pruning obsolete data
        while ((*lClientIt).second.subscribers[MSGID_KINECT].second.size() > (*lClientIt).second.buffersInfo[MSGID_KINECT]) {
...
int subscribeTo(msgSubscribe* info, ros::NodeHandle nodeRos, matlabClient &in_client){
        ...
        ROS_INFO("Reception d'une demande d'inscription pour le capteur %02X.\nFrequence d'acquisition : %i\nFrequence de polling : %i\nTaille du buffer circulaire : %i", info->typeCapteur, info->freqAcquisition, info->freqSend, info->bufferSize);
        ...
        if(info->freqAcquisition == 0){
            ...
            if(info->bufferSize == 0){
                ...
                if(!otherClientConnected){
                    ROS_WARN("Aucun client actuellement connecte au capteur %02X, inscription seulement ignoree", info->typeCapteur);
                    return -1;
                } 
                info->bufferSize = bufferNewSize;
                ROS_INFO("Pas de taille de buffer envoyee, utilisation de la taille du precedent subscriber (%d echantillons)", bufferNewSize);
            }
            subOnly = true;
        }
                ...
                case MSGID_ADC:
                        if(!subOnly){
                            ...
                            if(!ros::service::call("/D_ADC/params", lParamsSetAdc)){
                                    ROS_ERROR("Le service de parametrage de l'ADC a renvoye une erreur (code %d).", lParamsSetAdc.response.ret);
                                    return -1;
                            }
                        }
                        else{
                ...
                case MSGID_IMU:
                        if(subOnly){
                            ...
                            if(!ros::service::call("/D_IMU/params", lParamsSetImu)){
                                    ROS_ERROR("Le service de parametrage de l'IMU en mode inscription seulement a renvoye une erreur (code %d).", lParamsSetImu.response.ret);
                                    return -1;
                            }
                        }
                        else{
                            ...
                            lParamsSetImu.request.subscribe = true;
                            if(!ros::service::call("/D_IMU/params", lParamsSetImu)){
                                    ROS_ERROR("Le service de parametrage de l'IMU a renvoye une erreur (code %d).", lParamsSetImu.response.ret);
                                    return -1;
                            }
                        }
                ...
                case MSGID_BATTERY:
                        if(subOnly){
                            ...
                            if(!ros::service::call("/D_Battery/params", lParamsSetBat)){
                                    ROS_ERROR("Le service de parametrage de la batterie a renvoye une erreur (code %d).", lParamsSetBat.response.ret);
                                    return -1;
                            }
                        }
                        else{
                ...
                case MSGID_WEBCAM:
                        if(subOnly){
                            ...
                            if(!ros::service::call("/D_Cam/params", lParamsSetCam)){
                                ROS_ERROR("Le service de parametrage de la camera a renvoye une erreur (code %d).", lParamsSetCam.response.ret);
                                return -1;
                            }
                        }
                        else{
                ...
                case MSGID_WEBCAM_STEREO:
                        if(subOnly){
                            ...
                            if(!ros::service::call("/D_CamStereo/params", lParamsSetStereoCam)){
                                ROS_ERROR("Le service de parametrage de la camera stereo a renvoye une erreur (code %d).", lParamsSetStereoCam.response.ret);
                                return -1;
                            }
                        }
                        else{
                ...
                case MSGID_COMPUTER:
                        if(subOnly){
                            ...
                            if(!ros::service::call("/D_ComputerInfo/params", lParamsSetComputer)){
                                ROS_ERROR("Le service de parametrage des statistiques de l'ordinateur a renvoye une erreur (code %d).", lParamsSetComputer.response.ret);
                                return -1;
                            }
                        }
                        else{
        ...
        switch(info->typeCapteur){
                case MSGID_ADC:
                        ...
                        if(!ros::service::call("/D_ADC/params", lParamsUnsetAdc)){
                                ROS_ERROR("Le service de parametrage de l'ADC a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetAdc.response.ret);
                                return -1;
                        }
                        break;
                case MSGID_IMU:
                        ...
                        if(!ros::service::call("/D_IMU/params", lParamsUnsetImu)){
                                ROS_ERROR("Le service de parametrage de l'IMU a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetImu.response.ret);
                                return -1;
                        }
                        break;
                case MSGID_BATTERY:
                        ...
                        if(!ros::service::call("/D_Battery/params", lParamsUnsetBattery)){
                                ROS_ERROR("Le service de parametrage de la batterie a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetBattery.response.ret);
                                return -1;
                        }
                        break;
                case MSGID_WEBCAM:
                        ...
                        if(!ros::service::call("/D_Cam/params", lParamsUnsetCam)){
                                ROS_ERROR("Le service de parametrage de la camera a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetCam.response.ret);
                                return -1;
                        }
                    break;
                case MSGID_WEBCAM_STEREO:
                        ...
                        if(!ros::service::call("/D_CamStereo/params", lParamsUnsetStereoCam)){
                                ROS_ERROR("Le service de parametrage des cameras stereo a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetStereoCam.response.ret);
                                return -1;
                        }
                    break;
                case MSGID_COMPUTER:
                        ...
                        if(!ros::service::call("/D_ComputerInfo/params", lParamsUnsetComputer)){
                                ROS_ERROR("Le service de parametrage des stats de l'ordinateur a renvoye une erreur (code %d) lors de la deconnexion.", lParamsUnsetComputer.response.ret);
                                return -1;
                        }
                        break;
                case MSGID_GPS:
...
int sendSerialCmd(msgSerialCmd* info, matlabClient &in_client, char* answer, char& answerSize) {
        ...
        ROS_INFO("Serial received on Logico Agent : speed = %d / parity = %d / stop = %d / sendLength = %d / readLength = %d / readTimeoutSec = %d / readTimeoutMicro = %d", info->speed, info->parity, info->stopBits, info->sendLength, info->readLength, info->readTimeoutSec, info->readTimeoutMicro);
        ...
        for(unsigned int i=0; i < info->sendLength; i++)
...
int sendDataToClient(int socketFd, msgHeader* inHeader, const char* bufferOut, int sizeBuffer, int compressionType){
    ...
    if(compressionType == MSGID_HEADER_NOCOMPRESSION){
        ...
        if (sizeBuffer > 0) {
            ...
            int retVal = ezcompress((unsigned char*)bufferCompressOut, &sizeCompress, (unsigned char*)bufferOut, sizeBuffer);
            if(retVal < 0){
    ...
    ROS_INFO("Data sent by sendDataToClient (%i bytes envoyes, %u bytes devraient l'avoir ete, header.type = %X)", sendBytes, sizeof(msgHeader)+inHeader->compressSize, inHeader->type);
        ...
        for (i = 0; i < FD_SETSIZE; ++i) {
            if (FD_ISSET (i, &read_fd_set)) {
                if (i == sd) {
                  ...
                  ROS_INFO("Server: connect from host %s, port %u.\n", inet_ntoa (cliAddr.sin_addr), ntohs (cliAddr.sin_port));
                  FD_SET (newcon, &active_fd_set);
                    ...
                    while (recvBytes < sizeof(msgHeader)) {
                        ...
                        if(recvReturn == -1){
                            // Timeout
                            ROS_ERROR("Timeout on header reception (Receibed %db / %db expected)", recvBytes, sizeof(msgHeader));
                            break;
                        }
                    ...
                    while (recvBytes < ProtocolMsgSize[lHeader.type]*lHeader.size) {
                        ...
                        if(recvReturn == -1){
                            // Timeout
                            ROS_ERROR("Timeout on message reception (header correctly received) : %db received, %db expected!", recvBytes, ProtocolMsgSize[lHeader.type]*lHeader.size);
                            delete[] msg;
                            break;
                        }
                        else if(recvReturn == 0){
                    ...
                    switch (lHeader.type) {
                        ...
                        case MSGID_KINECT_DEPTH:
                            ...
                            if(clients[i].subscribers[lHeader.type].second.size() == 0) {
                                ...
                                int imgSizeBytes = ((msgCamInternal*)clients[i].subscribers[lHeader.type].second.front())->sizeData;

                                int sendSize = nbrImgSend * sizeof(msgCam) + nbrImgSend * imgSizeBytes;

                                lAnswerHeader.size = nbrImgSend;
                                lAnswerHeader.info = (((msgCamInternal*)clients[i].subscribers[lHeader.type].second.front())->width) / 20;
                                ...
                                for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                    ...
                                    memcpy(lAnswer + k * sizeof(msgCam),
                                           &lCamData,
                                           sizeof(msgCam));

                                    // Image copy
                                    if (lHeader.type == MSGID_KINECT) {
                                        memcpy(lAnswer + nbrImgSend * sizeof(msgCam) + k * imgSizeBytes,
                        ...
                        case MSGID_HOKUYO:
                            ...
                            if(clients[i].subscribers[MSGID_HOKUYO].second.size() == 0){
                                ...
                                int rangesSizeBytes = ((msgHokuyoInternal*)clients[i].subscribers[MSGID_HOKUYO].second.front())->sizeData;
                                ...
                                for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                    ...
                                    // Image copy
                                    memcpy(lAnswer + nbrCaptureSend*sizeof(msgHokuyo) + k*rangesSizeBytes, currentStruct->cptr, rangesSizeBytes);
                        ...
                        case MSGID_BATTERY:
                            ...
                            for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                memcpy(lAnswer + k * sizeof(msgBattery), clients[i].subscribers[lHeader.type].second.front(), sizeof(msgBattery));
                                delete (msgBattery*)clients[i].subscribers[lHeader.type].second.front();
                                clients[i].subscribers[lHeader.type].second.pop();
                            }
                            sendDataToClient(i, &lAnswerHeader, lAnswer, lAnswerHeader.size * sizeof(msgBattery), clients[i].compression);
                            delete lAnswer;
                            break;
                        case MSGID_GPS:
                            ...
                            for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                memcpy(lAnswer + k * sizeof(msgGps), clients[i].subscribers[lHeader.type].second.front(), sizeof(msgGps));
                                delete (msgGps*)clients[i].subscribers[lHeader.type].second.front();
                                clients[i].subscribers[lHeader.type].second.pop();
                            }
                            sendDataToClient(i, &lAnswerHeader, lAnswer, lAnswerHeader.size * sizeof(msgGps), clients[i].compression);
                            delete lAnswer;
                            break;
                        case MSGID_COMPUTER:
                            ...
                            for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                memcpy(lAnswer + k * sizeof(msgComputer), clients[i].subscribers[lHeader.type].second.front(), sizeof(msgComputer));
                                delete (msgComputer*)clients[i].subscribers[lHeader.type].second.front();
                                clients[i].subscribers[lHeader.type].second.pop();
                            }
                            sendDataToClient(i, &lAnswerHeader, lAnswer, lAnswerHeader.size * sizeof(msgComputer), clients[i].compression);
                        ...
                        case MSGID_ADC:
                            ...
                            for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                memcpy(lAnswer + k * sizeof(msgAdc), clients[i].subscribers[lHeader.type].second.front(), sizeof(msgAdc));
                                delete (msgAdc*)clients[i].subscribers[lHeader.type].second.front();
                                clients[i].subscribers[lHeader.type].second.pop();
                            }
                            
                            sendDataToClient(i, &lAnswerHeader, lAnswer, lAnswerHeader.size * sizeof(msgAdc), clients[i].compression);
                            delete lAnswer;
                            break;
                        case MSGID_IMU:
                            ...
                            for (unsigned int k = 0; k < lAnswerHeader.size; k++) {
                                memcpy(lAnswer + k * sizeof(msgImu), clients[i].subscribers[lHeader.type].second.front(), sizeof(msgImu));
                                delete (msgImu*)clients[i].subscribers[lHeader.type].second.front();
                                clients[i].subscribers[lHeader.type].second.pop();
                            }
                            sendDataToClient(i, &lAnswerHeader, lAnswer, lAnswerHeader.size * sizeof(msgImu), clients[i].compression);
                            delete lAnswer;
                            break;
                        default:
